...
def create_folders_periodically(folder_list: list, duration_seconds: int = 1) -> None:
    '''
    ...
    while folder_list:
        ...
        name_path.mkdir(exist_ok=True)
        logger.info(f"Created folder: {name_path}")
        logger.info(f"Sleeping for {duration_seconds} second(s).")
...
def main() -> None:
    ...
    logger.info(f"Byline: {utils_jordan.get_byline()}")
